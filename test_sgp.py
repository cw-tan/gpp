...
def func(x):
    return torch.cos(0.5*x) - 0.3 * x + 0.1 * torch.exp(0.3 * x) + 0.05 * torch.rand(x.shape)

L = 10
x_test = torch.linspace(-L - 1, L + 1, 1000, dtype=torch.float64)

kernel = SquaredExponentialKernel()
SGP = SparseGaussianProcess(1, kernel, decomp_mode='v', sgp_mode='vfe', init_noise=0.05)

print(SGP.outputscale, SGP.noise)

x_train = L * (2 * torch.rand(50, dtype=torch.float64) - 1)
