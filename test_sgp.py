...
def func(x):
    return torch.cos(0.5*x) - 0.3 * x + 0.1 * np.exp(0.3 * x) + 0.5 * torch.rand(x.shape)

L = 10
x_test = torch.linspace(-L - 1, L + 1, 1000, dtype=torch.float64)


SGP = SparseGaussianProcess(1, invert_mode='v', sgp_mode='vfe')
...
x_sparse = SGP.remove_duplicates(x_sparse, x_sparse, tol=1e-7)
...
for i in range(10):
    ...
    x_sparse_new = SGP.remove_duplicates(x_sparse_new, x_sparse_new, tol=1e-7)
    x_sparse_new = SGP.remove_duplicates(SGP.sparse_descriptors, x_sparse_new, tol=1e-7)
