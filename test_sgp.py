...
SGP = SparseGaussianProcess(1)

plt.plot(x_test, func(x_test), '-k', label=r'$f(x)$')
x_train = L * (2 * torch.rand(60, dtype=torch.float64) - 1)
y_train = func(x_train)

# use 25 % of full training points as sparse points
x_sparse = x_train[torch.randperm(len(x_train))[:(int(len(x_train) / 4))]]
#x_sparse[-1] = x_sparse[0] + 1e-7

SGP.update_model(x_train, y_train, x_sparse)
steps = SGP.optimize_hyperparameters()
print(steps)
