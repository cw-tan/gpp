...
class TestKernelFunctions(unittest.TestCase):
    ...
    def test_duplicate_handling(self):
        ...
        duplicate_ids = self.kernel.get_duplicate_ids(torch.atleast_2d(x1),
                                                      torch.atleast_2d(x2),
                                                      tol=1e-8)
        self.assertTrue(torch.equal(duplicate_ids, torch.tensor([0, 3, 4, 8])))
        x2_clean = self.kernel.remove_duplicates(torch.atleast_2d(x1), torch.atleast_2d(x2), tol=1e-8)
        self.assertTrue(torch.equal(x2_clean, torch.atleast_2d(torch.tensor([2.25, 2.5, 5.5, 6, 7],
                                                                            dtype=torch.float64))))

        duplicate_ids = self.kernel.get_duplicate_ids(torch.atleast_2d(x2),
                                                      torch.atleast_2d(x1),
                                                      tol=1e-8)
        self.assertTrue(torch.equal(duplicate_ids, torch.tensor([0, 2, 3, 5])))
        x1_clean = self.kernel.remove_duplicates(torch.atleast_2d(x2), torch.atleast_2d(x1), tol=1e-8)
        ...
        duplicate_ids = self.kernel.get_duplicate_ids(torch.atleast_2d(x3),
                                                      torch.atleast_2d(x3),
                                                      tol=1e-8)
